...
class Sximo extends Model {
	...
	public static function getRows( $args )
	{
	   ...
	   $key = with(new static)->primaryKey;
		...
		$table = with(new static)->table;
		if($global == 0 )
				$params .= " AND ( {$table}.entry_by_access ='".\Session::get('uid')."' OR {$table}.entry_by_access IN ( select subcontratistas.entry_by_access from tbl_contratistas inner join tbl_subcontratistas on tbl_subcontratistas.IdContratista = tbl_contratistas.IdContratista inner join tbl_contratistas as subcontratistas on tbl_subcontratistas.SubContratista = subcontratistas.IdContratista where tbl_contratistas.entry_by_access = '".\Session::get('uid')."' ) )";
	...
	public  function insertRow( $data , $id)
	{
	   ...
	   $key = with(new static)->primaryKey;
	    if($id == NULL )
        {
            ...
            // Insert Here
            if(isset($data['entry_by'])) $data['entry_by'] = \Session::get('uid');
			...
			if(isset($data['entry_by'])) unset($data['entry_by']);
			if(isset($data['updatedOn'])) $data['updatedOn'] = date("Y-m-d H:i:s");
    ...
    static function getComboselect( $params , $limit =null, $parent = null)
    {
        ...
        if(count($limit) >=3)
        {
            ...
            //Agregar condiciones aqui para que sea transversal....
            $table = $params[0];
            if(count($parent)>=2 )
            {
            	$row =  \DB::table($table)->where($parent[0],$parent[1])->orderBy($table.".".$fields[0],"asc")->get();
            } else  {
	            $row =  \DB::table($table)->orderBy($table.".".$fields[0],"asc")->get();
            }
        }
