...
class SiteHelpers
{
	static public function currNumber($num){
		return "CLP ".number_format($num, 0, ',', '.');
	}

	public static function menus( $position ='top',$active = '1')
	{

		if (\Session::get('gid')==15){
			$lintGroupUser = \MySourcing::GroupUser(\Session::get('uid'));
			if ($lintGroupUser!=\Session::get('gid')){
				session(['gid' => $lintGroupUser]);
			}
		}
		$data = array();
	...
	public static function menus_api( $position ='top',$active = '1')
    {
        if (\Session::get('gid')==15){
            $lintGroupUser = \MySourcing::GroupUser(\Session::get('uid'));
            if ($lintGroupUser!=\Session::get('gid')){
        ...
        foreach ($menu as $row)
        {
            ...
            if($is_allow ==1)
            {
                    ...
                    foreach ($menus2 as $row2)
                    {
                        ...
                        if($is_allow ==1)
                        {
                            $menu2 = array(
                                    'menuId'        => $row2->menu_id,
                                    'module'        => $row2->module,
                                    'menuType'      => $row2->menu_type,
                                    'url'           => $row2->url,
                                    'menuName'      => $row2->menu_name,
                                    'menuLang'      => json_decode($row2->menu_lang,true),
                                    'menuIcons' => $row2->menu_icons,
                                    'childs'        => array()
                                );
                            $menus3 = self::nestedMenu($row2->menu_id , $position , $active);
                            if(count($menus3) > 0 )
                                ...
                                foreach ($menus3 as $row3)
                                {
                                    ...
                                    if($is_allow ==1)
                                    {
                                        $menu3 = array(
                                                'menuId'        => $row3->menu_id,
                                                'module'        => $row3->module,
                                                'menuType'      => $row3->menu_type,
                                                'url'           => $row3->url,
                                                'menuName'      => $row3->menu_name,
                                                'menuLang'      => json_decode($row3->menu_lang,true),
                                                'menuIcons' => $row3->menu_icons,
                                                'childs'        => array()
                                            );
                                        $child_level_3[] = $menu3;
                                    }
                ...
                $level = array(
                        ...
                        'menuLang'      => json_decode($row->menu_lang,true),
                        'menuIcons' => $row->menu_icons,
                        'childs'        => $child_level
                    );
                $data[] = $level;
            }
...
public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null)
{
	...
	public static  function bulkForm( $field, $forms = array(), $value ='',$bulkLabel='',$lintIdUser='')
	{
		$type = '';
		$bulk ='true';
		$bulk = ($bulk == true ? '[]' : '');
		...
		switch($type)
		{
			...
			case 'text';
				$form = "<input  type='text' id='".$field."' name='".$field."{$bulk}' class='form-control input-sm ".$field."' $mandatory value='{$value}'/>";
				break;

			case 'text_date';
				$form = "<input  type='text' id='".$field."' name='$field{$bulk}' class='date form-control input-sm ".$field."' $mandatory value='{$value}'/> ";
				break;
			...
			case 'select';
				...
				if($option['opt_type'] =='external')
				{
					...
					if ($lintIdUser && $option['lookup_table']=='tbl_personas'){
					...
					for($i=0; $i<count($opt);$i++)
					{
						$selected = '';
						$row =  explode(":",$opt[$i]);
						if($value == trim($row[0])) $selected ='selected="selected"';
						$opts .= "<option $selected value ='".trim($row[0])."' > ".$row[1]." </option> ";
					}


				}
				$form = "<select  id='".$field."' name='$field{$bulk}'  class='form-control ".$field." select3' $mandatory >
							<option value=''> -- Select  -- </option>
							$opts
						</select>";
				break;
		...
		if($format_as =='image')
		{
			...
			if($format_value =='')
			{
				if(defined('CNF_DATE') && CNF_DATE !='' )
				{
					...
					$value = $date->format("".CNF_DATE."");
				}
			} else {
                if ($value!='' && $value && $value!='0000-00-00 00:00:00' && $value!= '0000-00-00'){
					$date = new DateTime($value);
					$value = $date->format("$format_value");
                }else{
                	$value = '-';
                }
			}
	...
	public static function themeSourcingOption()
	{

		$path = public_path('sximo/customer/');
		$lang = scandir($path);
		$t = array();
		foreach($lang as $value) {
			if($value === '.' || $value === '..') {continue;}
