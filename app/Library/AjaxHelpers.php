...
class AjaxHelpers
{
	...
	static public function buttonAction( $module , $access , $id , $setting, $opcional = NULL)
	{
		...
		if($module=='moduloalertasmaster'){
			$html .= '<a href="#" onclick="ajaxApproveInLine(\''.$id.'\',5); return false;"  class="btn btn-xs btn-white tips" title=" Procesar"><i class="fa  fa-bell"></i></a>';
		}
		if ($opcional==3){
			$onclick = " onclick=\"ajaxViewDetail('#".$module."',this.href); return false; \"" ;
			if($setting['form-method'] =='modal')
					$onclick = " onclick=\"SximoModalPeople(this.href,'Edit Form'); return false; \"" ;
			$html .= ' <a href="'.URL::to($module.'/acciones/'.$id).'" '.$onclick.'  class="btn btn-xs btn-white tips" title="Acciones"><i class="fa fa-cog"></i></a>';
		}
		$html .= '</div>';
		return $html;
	}
	...
	static public function buttonActionCreate( $module  ,$method = 'newpage')
	{
		...
		$html = '
			<a href="'.URL::to($module.'/update').'" class="tips btn vs-button--default white"  title="'.Lang::get('core.btn_create').'" '.$onclick.'>
