...
class SximoforumController extends Controller {
	...
	public function getIndex( Request $request )
	{
		...
		foreach($results['rows'] as $forum)
		{
			$theard = array();
			$theards = $this->model->theard($forum->ForumID);
			foreach ( $theards as $th) {
				$theard[] = array(
	...
	function postSave( Request $request)
	{
		...
		if ($validator->passes()) {
			...
			{
				\SiteHelpers::auditTrail( $request , 'New Data with ID '.$id.' Ha sido ingresado !');
			} else {
				\SiteHelpers::auditTrail($request ,'Data with ID '.$id.' Ha sido actualizado !');
			}

			return Redirect::to($return)->with('messagetext',\Lang::get('core.note_success'))->with('msgstatus','Exitoso!');
	...
	public function postDelete( Request $request)
	{
		...
		if(count($request->input('ids')) >=1)
		{
			...
			\SiteHelpers::auditTrail( $request , "ID : ".implode(",",$request->input('ids'))."  , Ha sido borrado satisfactoriamente");
			// redirect
			return Redirect::to('sximoforum')
        		->with('messagetext', \Lang::get('core.note_success_delete'))->with('msgstatus','Exitoso!'); 
	...
	public function postSavepost( Request $request )
	{
		...
		if($request->input('PostID') == '')
		{
			...
			Mail::send('sximoforum.emailpost', $data, function ($message) {
			    $message->to(CNF_EMAIL)->subject('Sximo Forum new Post !');
			});			
		} else {
	...
	public function getDeletetopic( Request $request , $id = 0 )
	{
		...
		if($id != 0 )
		{
			\DB::table('forum_categories')->where('CategoryID',$id)->delete();
			\DB::table('forum_posts')->where('CategoryID',$id)->delete();
			// redirect
			return Redirect::to('sximoforum')
        		->with('messagetext', \Lang::get('core.note_success_delete'))->with('msgstatus','success'); 
