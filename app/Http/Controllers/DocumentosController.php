...
class DocumentosController extends Controller {
	...
	public function getShowlist( Request $request){
		...
		if ($lintLevelUser != 1 && $lintLevelUser!=7){
			...
			#$filter = " AND tbl_tipos_documentos.id = '".$lintGroupUser."' ";
                        $filter = " AND tbl_tipos_documentos.IdTipoDocumento IN ( SELECT idTipoDocumento FROM tbl_tipo_documento_perfil WHERE IdPerfil = '".$lintGroupUser."') ";
		}else{
			$filter = " ";
		}
      ...
      //  var_dump(count($results));
		...
		foreach ($results['rows'] as $row) {
			...
			if ($row->IdEstatus==5){
                ...
                if (!(intval($tipo)==3 || intval($tipo)==6 || intval($tipo)==21 || intval($tipo)==26 || intval($tipo)==31)){
                    if ($estatusDoc==2){
                        if ($docRel){
                        	//var_dump($docRel);
                            $larrResultTemp['action'] = '<div class=" action dropup"><a href="'.\URL::to('documentos/update/'.$docRel->IdDocumento).'" onclick="SximoModal(this.href,\'Edit Form\'); return false; "  class="btn btn-xs btn-white tips" title="Renovar"><i class="fa  fa-calendar-times-o"></i></a></div>';
                        }else{
                            $larrResultTemp['action'] = '<div class=" action dropup"><a href="'.\URL::to('documentos/update/'."-".$id).'" onclick="SximoModal(this.href,\'Edit Form\'); return false; "  class="btn btn-xs btn-white tips" title="Renovar"><i class="fa  fa-calendar-times-o"></i></a></div>';
                        }
                    }
                    else{
                        $now = new \DateTime();
                        ...
                        if ($diff->days <= $vencimiento){
                            if ($docRel)
                                $larrResultTemp['action'] = '<div class=" action dropup"><a href="'.\URL::to('documentos/update/'.$docRel).'" onclick="SximoModal(this.href,\'Edit Form\'); return false; "  class="btn btn-xs btn-white tips" title="Renovar"><i class="fa  fa-calendar-times-o"></i></a></div>';
                            else
                                $larrResultTemp['action'] = '<div class=" action dropup"><a href="'.\URL::to('documentos/update/'."-".$id).'" onclick="SximoModal(this.href,\'Edit Form\'); return false; "  class="btn btn-xs btn-white tips" title="Renovar"><i class="fa  fa-calendar-times-o"></i></a></div>';
                        }
                        else{
	...
	function getUpdate(Request $request, $id = null)
	{
		...
		if ($lobjDatosDocumento->TipoDocumento->IdProceso == 17) { //Multa generales
			return view('documentos.multasgeneral',$this->data);
		}elseif ($lobjDatosDocumento->TipoDocumento->IdProceso == 77) { // Multas chile
			return view('documentos.formmultas',$this->data);
		}elseif ($lobjDatosDocumento->TipoDocumento->IdProceso == 79) { //Situación Tributaria
			return view('documentos.situaciontributaria',$this->data);
		}else{
	...
	function postSave( Request $request, $id =0)
	{
		...
		$ldatFechaEmision = $request->input('FechaEmision');
		$lobjDocumento = $lobjMyDocumentos::getDatos();
		//$request->bulk_IdTipoDocumentoValor = false;

		\Log::info(Input::file('DocumentoURL')); 
		\Log::info($request->input('IdDocumento')); 
		...
		if ($lobjDocumento->TipoDocumento->IdProceso == 17){
		...
		}elseif ($lobjDocumento->TipoDocumento->IdProceso == 140){
			$larrResult = \MyLoadbatch::LoadExcelABT($lobjArchivoDocumento,$lobjDocumento->IdTipoDocumento, $request->input('IdDocumento'));
		}
		else{
			...
			if ($larrResult['code']==1){
				if ($request->bulk_IdTipoDocumentoValor){
					$larrResult = $lobjMyDocumentos::loadvalues($request->bulk_IdTipoDocumentoValor, $request->bulk_Valor);
				}

				if ($lobjDocumento->TipoDocumento->IdProceso == 4) {
					$idPersona = $lobjDocumento->IdEntidad;
					$anot = $request->input('anotacion');
					$updatedOn = date('Y-m-d');

					\DB::table('tbl_anotaciones')->where('IdPersona',$idPersona)->update(
							['IdConceptoAnotacion' => $anot, 'updatedOn'=> $updatedOn ]);

				}

				if ($lobjDocumento->TipoDocumento->IdProceso == 1 || $lobjDocumento->TipoDocumento->IdProceso == 112){

					$datetime1 = new \DateTime(\MyFormats::FormatoFecha($request->input('FechaEmision')));
	 				$datetime2 = new \DateTime('now');

	 				$interval = $datetime2->diff($datetime1)->format("%a");

					\DB::table('tbl_carga_f30_1')->insert(['IdDocumento'=>$request->input('IdDocumento'),'FechaEmision'=>\MyFormats::FormatoFecha($request->input('FechaEmision'))]);

					if($interval>60 and $lobjDocumento->TipoDocumento->IdProceso == 1){
						\DB::table('tbl_documentos')->where('IdDocumento',$request->input('IdDocumento'))->update(['IdEstatus'=>3,'Resultado'=>'De acuerdo a la información proporcionada, el documento se encuentra expirado y no podrá ser validado en la web de la Dirección del Trabajo, debe reemitirlo']);
					}else{
	...
	function moverAlHistorico(RUT $rut){
		//Descripcion: permite mover los documentos de una persona al historico a excepcion de "preguntar"
/*$documentos = DB::table('tbl_documentos')
						->join('')*/

					//	TODO: terminar la consulta para cambiar los documentos al historico.
		// SELECT b.* FROM tbl_personas a
		// INNER JOIN tbl_documentos b ON a.IdPersona = b.IdEntidad
		// WHERE b.Entidad = 3
		// AND a.RUT = '26103250-3'

	}
	public function f30Tesoreria(){

		// Lo que se necesita es que para esos dos documentos (individualmente) se busque el aprobado más reciente
		// y se copie en todas las solicitudes de ese mismo tipo de documento para ese mismo contratista (ambos son entidad 1) que tenga en estado 1 o 3
		// y con fecha emisión anterior a la fecha del que tiene aprobado. Además, esas solicitudes donde se copió el documento tienen que pasar a estado 5 con el comentario genérico

		//f30

        $Contra = \DB::table('tbl_documentos')
                        ->where('tbl_documentos.IdTipoDocumento', 110)
                        ->where('tbl_documentos.Entidad', 1)
                        ->where('tbl_documentos.IdEstatus', 5)
                        ->GroupBy('tbl_documentos.IdEntidad')
                        ->get();

      	foreach ($Contra as $key => $Contratista) {
			  $documentoMax = \DB::table('tbl_documentos')
			  				  ->where('tbl_documentos.IdEntidad', $Contratista->IdEntidad)
							  ->where('tbl_documentos.Entidad',1)
							  ->where('tbl_documentos.IdTipoDocumento',110)
							  ->where('tbl_documentos.IdEstatus',5)
							  ->orderBy('tbl_documentos.FechaEmision', 'desc')->first();

			if(!is_null($documentoMax)){
				$documentoAnteriores = \DB::table('tbl_documentos')
				->where('tbl_documentos.FechaEmision','<',$documentoMax->FechaEmision )
				->where('tbl_documentos.Entidad', 1 )
				->where('tbl_documentos.IdTipoDocumento', 110)
				->where('tbl_documentos.IdEntidad', $documentoMax->IdEntidad)
				->whereIn('tbl_documentos.IdEstatus', [1,3])
				->update([
					'DocumentoURL' => $documentoMax->DocumentoURL,
					'IdEstatus' => 5,
					'Resultado' => "Reemplazado con documento cargado para el periodo ".$documentoMax->FechaEmision
				]);
        ...
        $Contra = \DB::table('tbl_documentos')
                        ->where('tbl_documentos.IdTipoDocumento', 114)
                        ->where('tbl_documentos.Entidad', 1)
                        ->where('tbl_documentos.IdEstatus', 5)
                        ->GroupBy('tbl_documentos.IdEntidad')
                        ->get();
