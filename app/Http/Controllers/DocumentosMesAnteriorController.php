...
use App\Models\DocumentosMesAnterior;
...
class documentosmesanteriorController extends Controller {

	protected $layout = "layouts.main";
	protected $data = array();
	public $module = 'documentosMesAnterior';
	static $per_page	= '10';

	public function __construct()
	{
		parent::__construct();
		$this->model = new DocumentosMesAnterior();
		...
		$this->data = array(
			...
			'pageModule'		=> 'documentosMesAnterior',
			'pageUrl'			=>  url('documentosMesAnterior'),
	...
	public function getIndex()
	{
		...
		return view('documentosMesAnterior.index',$this->data);
	}
	...
	public function getShowlist( Request $request){
		...
		if ($lintLevelUser != 1 && $lintLevelUser!=7){
            ...
            $filter = " AND tbl_tipos_documentos.IdTipoDocumento IN ( SELECT idTipoDocumento FROM tbl_tipo_documento_perfil WHERE IdPerfil = '".$lintGroupUser."') AND tbl_documentos.FechaEmision=concat(substring(DATE_SUB(curdate(), INTERVAL 1 MONTH),1,8),'01') AND tbl_tipos_documentos.Acreditacion = 0 ";
		}else{
			$filter = " AND tbl_tipos_documentos.Acreditacion = 0 ";
		}
		...
		foreach ($results['rows'] as $row) {
			...
			if ($row->IdEstatus==5){
                ...
                if (!(intval($tipo)==3 || intval($tipo)==6 || intval($tipo)==21 || intval($tipo)==26 || intval($tipo)==31)){
                    if ($estatusDoc==2){
                        if ($docRel)
                            $larrResultTemp['action'] = '<div class=" action dropup"><a href="'.\URL::to('documentosMesAnterior/update/'.$docRel).'" onclick="SximoModal(this.href,\'Edit Form\'); return false; "  class="btn btn-xs btn-white tips" title="Renovar"><i class="fa  fa-calendar-times-o"></i></a></div>';
                        else
                            $larrResultTemp['action'] = '<div class=" action dropup"><a href="'.\URL::to('documentosMesAnterior/update/'."-".$id).'" onclick="SximoModal(this.href,\'Edit Form\'); return false; "  class="btn btn-xs btn-white tips" title="Renovar"><i class="fa  fa-calendar-times-o"></i></a></div>';
                    }
                    else{
                        $now = new \DateTime();
				...
				if (isset($larrEncuentas[$row->IdTipoDocumento])){
					...
					$larrResultTemp['action'] = '<div class=" action dropup"><a href="'.\URL::to('documentosMesAnterior/update/'.$id).'" onclick="SximoModal(this.href,\'Edit Form\'); return false; "  class="btn btn-xs btn-white tips" title="'.\Lang::get('core.btn_edit').'"><i class="fa  fa-upload"></i></a></div>';
				}
			}
	...
	public function postData( Request $request)
	{
		...
		return view('documentosMesAnterior.table',$this->data);
	...
	function getUpdate(Request $request, $id = null)
	{
		...
		if ($lobjDatosDocumento->TipoDocumento->IdProceso == 17) { //Multa generales
			return view('documentosMesAnterior.multasgeneral',$this->data);
		}elseif ($lobjDatosDocumento->TipoDocumento->IdProceso == 77) { // Multas chile
			return view('documentosMesAnterior.formmultas',$this->data);
		}elseif ($lobjDatosDocumento->TipoDocumento->IdProceso == 79) { //Situación Tributaria
			return view('documentosMesAnterior.situaciontributaria',$this->data);
		}else{
	        return view('documentosMesAnterior.form',$this->data);
		}
	...
	public function getShow( $id = null)
	{
		...
		if($row)
		{
			...
			return view('documentosMesAnterior.view',$this->data);
	...
	public static function display( )
	{
		...
		$model  = new documentosMesAnterior();
		$info = $model::makeInfo('documentosMesAnterior');
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('documentosMesAnterior.public.view',$data);
			}
			...
			return view('documentosMesAnterior.public.index',$data);
		}
