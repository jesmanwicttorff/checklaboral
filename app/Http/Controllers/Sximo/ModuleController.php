...
class ModuleController extends Controller {
	...
	public function getIndex( Request $request)
	{
        ...
        if (!is_null($request->input('t')) && $request->input('t') == "core")
        {
            $rowData = \DB::table('tb_module')->where('module_type','=','core')
                    ->orderby('module_title','asc')->get();
            $type = 'core';
        }elseif (!is_null($request->input('t')) && $request->input('t') == "checklaboral") {
            $rowData = \DB::table('tb_module')->where('module_type','=','checklaboral')
                    ->orderby('module_title','asc')->get();
            $type = 'checklaboral';
        } else {
            $rowData = \DB::table('tb_module')->where('module_type','!=','core')
    ...
    function getCreate( Request $request )
    {
        ...
        if (!is_null($request->input('t')) && $request->input('t') == "checklaboral") {
            $this->data['type'] = 'checklaboral';
        } else {
            $this->data['type'] = 'addon';
        }
    ...
    function postCreate( Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            if ( $request->input('type') == 'checklaboral' ) {
   ...
   function getPermission( $id )
    {
            ...
            if ( $id=="contratistas"){
                $tasks['is_report_view'] = 'Reporte ';
                $tasks['is_upload_massive'] = 'Carga Masiva ';
            }elseif ($id=="contratos" ){
                $tasks['is_action_button'] = 'Acciones ';
                $tasks['is_report_view'] = 'Reporte ';
                $tasks['is_upload_massive'] = 'Carga Masiva ';
            }elseif ($id=="personas" ){
                $tasks['is_action_button'] = 'Acciones ';
    ...
    public function postSavepermission( Request $request)
    {
        ...
        if ( $row->module_name =="contratistas"){
            $tasks['is_report_view'] = 'Reporte ';
            $tasks['is_upload_massive'] = 'Carga Masiva ';
        }elseif ($row->module_name=="contratos" ){
            $tasks['is_action_button'] = 'Acciones ';
            $tasks['is_report_view'] = 'Reporte ';
            $tasks['is_upload_massive'] = 'Carga Masiva ';
        }elseif ($row->module_name=="personas" ){
            $tasks['is_action_button'] = 'Acciones ';
            $tasks['is_upload_massive'] = 'Carga Masiva ';
        }
        /* Update permission global / own access new ver 1.1
   ...
   function configGrid ( $field , $alias , $type, $sort ) {
    ...
    public function getRebuild(Request $request, $id = 0)
    {
            ...
            $this->data['row'] = $row;
            $lstrDirectory = "";
            if ($row->module_type == 'checklaboral') {
                $lstrDirectory = 'checklaboral/';
            }
            $config = \SiteHelpers::CF_decode_json($row->module_config);
            ...
            /* End Master Detail */
            $dir = base_path().'/resources/views/'.$lstrDirectory.$class;
            $dirPublic = base_path().'/resources/views/'.$lstrDirectory.$class.'/public';
            $dirC = app_path().'/Http/Controllers/'.$lstrDirectory;
            $dirM = app_path().'/Models/'.$lstrDirectory;
            ...
            if ($row->module_type =='checklaboral') {
                ...
                $template = base_path().'/resources/views/sximo/module/template/checklaboral/';
            ...
            }elseif ($row->module_type =='generic' )
            {
    ...
    function createRouters()
    {
        ...
        foreach($rows as $row)
        {
            ...
            if ($row->module_type == "checklaboral"){
                ...
                $val .= "Route::controller('checklaboral/{$class}', 'checklaboral\\".$controller."');
                    ";
            }else{
                $controller = ucwords($row->module_name).'Controller';
