...
class UsersController extends Controller {
	...
	function postSave( Request $request, $id =0)
	{
		...
		if ($validator->passes() && $request["password"] === $request["password_confirmation"]) {
			...
			$lobjCompruebaMail = \DB::table('tb_users')
			->where("tb_users.id","!=",$request->input('id'))
			->where("tb_users.email","=",$request->input('email'))
			->get();

			$lobjCompruebaUser = \DB::table('tb_users')
			->where("tb_users.id","!=",$request->input('id'))
			->where("tb_users.username","=",$request->input('username'))
			->get();

			if ($lobjCompruebaUser || $lobjCompruebaMail) {
				return Redirect::to('core/users/update/'.$id)->with('messagetext',"El email ya se ha tomado")->with('msgstatus','error')
			...
			{
				return Redirect::to('core/users/update/'.$id)->with('messagetext','Las contraseñas no son iguales!')->with('msgstatus','error');
			} else {
	...
	function getBlast()
	{
		$this->data = array(
			'groups'	=> Groups::all(),
			'pageTitle'	=> 'Email MASIVO',
			'pageNote'	=> 'Se envía email a todos los usuarios de un(os) grupo(s) especifico(s)'
		);	
	...
	function postDoblast( Request $request)
	{
		...
		{	
			...
			if(!is_null($request->input('groups')))
			{
				...
				for($i=0; $i<count($groups); $i++)
				{
					...
					foreach($users as $row)
					{
						...
						{ 
					    	...
					    	//});
							//Se cambia para encolar los correos en lugar de tratar de procesarlos
							Mail::queue('core.users.email', $data, function ($message) use ($data) {
					    		$message->to($data['to'])->subject($data['subject']);
					    	});
				...
				return Redirect::to('core/users/blast')->with('messagetext','Total '.$count.' El mensaje ha sido enviado!')->with('msgstatus','success');

			}
			return Redirect::to('core/users/blast')->with('messagetext','Ningún mensaje ha sido enviado')->with('msgstatus','info');
