...
class AlterTriggerSetAccesosUpdateIdEstatusUsuario extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        DB::unprepared("CREATE DEFINER=`root`@`localhost` TRIGGER `setAccesos` 
                        ...
                        BEGIN
                             DECLARE lintIdEstatus, grupo, condicion, IdPer INT;
  ...
  IF (NEW.IdEstatus != OLD.IdEstatus) OR (NEW.IdEstatusDocumento != OLD.IdEstatusDocumento) THEN
	     SET lintIdEstatus := 2;
	      IF (NEW.Entidad = 1) THEN
                    ...
                    UPDATE tbl_accesos set IdEstatus = fnComprobarAccesos(IdPer), IdEstatusUsuario = fnComprobarAccesos(IdPer)
          ...
          ELSEIF (NEW.Entidad = 2) THEN
                    ...
                    UPDATE tbl_accesos
                    set IdEstatus =  fnComprobarAccesos(IdPer), IdEstatusUsuario = fnComprobarAccesos(IdPer)
                    where tbl_accesos.IdPersona IN (
                    Select tbl_contratos_personas.IdPersona 
                    from tbl_contratos_personas
