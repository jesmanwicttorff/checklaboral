...
class CreateFnCierreLaboralFunction extends Migration
{
    /**
     * Run the migrations.
     *
        ...
        DB::unprepared('CREATE DEFINER=`root`@`localhost` FUNCTION `fnCierreLaboral`(pdatPeriodo DATE, pbolSobreescribir INT) RETURNS varchar(256) CHARSET latin1
            BEGIN
              ...
              BEGIN
                 ...
                 INSERT INTO tbl_personas_maestro(periodo, idpersona, contrato_id, idcontratista, created_at, updated_at)
                 SELECT pdatPeriodo, tbl_personas.IdPersona, tbl_documentos.contrato_id, tbl_documentos.idcontratista, now(), now()
                 FROM tbl_personas
                 INNER JOIN tbl_documentos ON tbl_documentos.entidad = 3 and tbl_documentos.IdEntidad = tbl_personas.IdPersona and tbl_documentos.IdTipoDocumento = 7
                 ...
                 LEFT JOIN tbl_documento_valor ON tbl_documento_valor.IdDocumento = tbl_documentos.IdDocumento and tbl_documento_valor.IdTipoDocumentoValor = 171 and tbl_documento_valor.Valor BETWEEN pdatPeriodo AND LAST_DAY(pdatPeriodo);
              
                 -- insertamos finiquitadas en este periodo
                 INSERT INTO tbl_personas_maestro(periodo, idpersona, contrato_id, idcontratista, created_at, updated_at)
                 SELECT pdatPeriodo, tbl_personas.IdPersona, tbl_documentos_rep_historico.contrato_id, tbl_documentos_rep_historico.idcontratista, now(), now()
                 FROM tbl_personas
                 INNER JOIN tbl_documentos_rep_historico ON tbl_documentos_rep_historico.entidad = 3 and tbl_documentos_rep_historico.IdEntidad = tbl_personas.IdPersona and tbl_documentos_rep_historico.IdTipoDocumento = 7
              ...
              BEGIN
                 ...
                 INSERT INTO tbl_contrato_maestro(periodo, idcontratista, contrato_id, dotacion, costo_laboral, pasivo_laboral, created_at, updated_at)
                 SELECT DISTINCT pdatPeriodo, tbl_personas_maestro.idcontratista, tbl_personas_maestro.contrato_id, count(tbl_personas_maestro.idpersona), 0, 0, now(), now()
                 FROM tbl_personas_maestro
                 WHERE tbl_personas_maestro.periodo = pdatPeriodo
                 GROUP BY tbl_personas_maestro.idcontratista, tbl_personas_maestro.contrato_id;
